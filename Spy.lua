...
local function SpyNormalizePlayerName(name)
        if name then
                return string.gsub(name, " %- ", "-")
        end
end

local function SpyIsPlayerGUID(guid)
        if not guid then
                return false
        end
        local guidType = tonumber("0x"..strsub(guid, 3, 5))
        if not guidType then
                return false
        end
        local unitType = bit.band(guidType, 0x00F)
        return unitType == 0 or unitType == 8
end
...
--                              level = nil
                ...
                elseif UnitIsFriend("player", "target") and not UnitIsUnit("player", "target") then
                        name = SpyNormalizePlayerName(name)
                        Spy:AddFriendlyDetected(name, time())
                end
        end
end
...
--                              level = nil
                ...
                elseif UnitIsFriend("player", "mouseover") and not UnitIsUnit("player", "mouseover") then
                        name = SpyNormalizePlayerName(name)
                        Spy:AddFriendlyDetected(name, time())
                end
        end
...
function Spy:CombatLogEvent(_, timestamp, event, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
        if Spy.EnabledInZone then
                if srcName then
                        srcName = SpyNormalizePlayerName(srcName)
                end
                if dstName then
                        dstName = SpyNormalizePlayerName(dstName)
                end
                ...
                if bit.band(srcFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE and srcGUID and srcName and not SpyPerCharDB.IgnoreData[srcName] then
                        if SpyIsPlayerGUID(srcGUID) then
                                local learnt = false
                                local detected = true
                ...
                if bit.band(dstFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE and dstGUID and dstName and not SpyPerCharDB.IgnoreData[dstName] then
                        if SpyIsPlayerGUID(dstGUID) then
                                local learnt = false
                                local detected = true
                ...
                if srcFlags and srcGUID and srcGUID ~= UnitGUID("player") and srcName and srcName ~= Spy.CharacterName and not SpyPerCharDB.IgnoreData[srcName] and SpyIsPlayerGUID(srcGUID) and bit.band(srcFlags, COMBATLOG_OBJECT_REACTION_FRIENDLY) == COMBATLOG_OBJECT_REACTION_FRIENDLY then
                        Spy:AddFriendlyDetected(srcName, timestamp)
                end

                if dstFlags and dstGUID and dstGUID ~= UnitGUID("player") and dstName and dstName ~= Spy.CharacterName and not SpyPerCharDB.IgnoreData[dstName] and SpyIsPlayerGUID(dstGUID) and bit.band(dstFlags, COMBATLOG_OBJECT_REACTION_FRIENDLY) == COMBATLOG_OBJECT_REACTION_FRIENDLY then
                        Spy:AddFriendlyDetected(dstName, timestamp)
                end
