...
function Spy:CreateMainWindow()
        ...
        theFrame.TitleClick:EnableMouseWheel(true)              
                ...
                -- Create the Friendly Counter Text
                theFrame.CountFrame.FriendText = CountFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
                -- This anchors it to the LEFT of the enemy counter so they sit side-by-side
                theFrame.CountFrame.FriendText:SetPoint("RIGHT", theFrame.CountFrame.Text, "LEFT", -10, 0)
                theFrame.CountFrame.FriendText:SetFont("GameFontNormal", Spy.db.profile.MainWindow.RowHeight * 0.85, "OUTLINE")
                theFrame.CountFrame.FriendText:SetJustifyH("RIGHT")
                theFrame.CountFrame.FriendText:SetJustifyV("MIDDLE")
                theFrame.CountFrame.FriendText:SetTextColor(0, 1, 0, 1) -- Pure Green
                theFrame.CountFrame.FriendText:SetText("0")

                theFrame.FriendCountButton = CreateFrame("Button", nil, theFrame)
                theFrame.FriendCountButton:SetWidth(28)
                theFrame.FriendCountButton:SetHeight(Spy.db.profile.MainWindow.RowHeight)
                theFrame.FriendCountButton:SetAlpha(.0)
                theFrame.FriendCountButton:SetPoint("RIGHT", theFrame.CountFrame.Text, "LEFT", -10, 0)
                theFrame.FriendCountButton:SetScript("OnEnter", function(self)
                        GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
                        GameTooltip:AddLine("Nearby Friendlies", 1, 0.82, 0, 1)
                        GameTooltip:AddLine("The number of friendly players recently detected nearby.",1,1,1,1)
                        GameTooltip:Show()
                end)
                theFrame.FriendCountButton:SetScript("OnLeave", function(self)
                        GameTooltip:Hide()
                end)
                theFrame.FriendCountButton:SetFrameLevel(theFrame.FriendCountButton:GetFrameLevel() + 1)
...
function Spy:SetBar(num, name, desc, value, colorgroup, colorclass, tooltipData, opacity)
        ...
        -- 1. Combine Name and Level (Name first, then Level)
        local displayName = name
        if playerData and playerData.level then
                displayName = name.." ["..playerData.level.."]"
        end
        
        -- 2. Get Guild Name for the Right Text
        local displayGuild = ""
        if playerData and playerData.guild and playerData.guild ~= "" then
                displayGuild = playerData.guild
        end

        Row.StatusBar:SetValue(value)
        Row.LeftText:SetText(displayName) 
        Row.RightText:SetText(displayGuild) 
        Row.Name = name
        Row.TooltipData = tooltipData
        
        -- 3. Adjust width so they don't overlap
        Row.LeftText:SetWidth(Row:GetWidth() - Row.RightText:GetStringWidth() - 10)
