...
function Spy:UpdateFriendlyCount()
        local friendlyCount = 0
        for k in pairs(Spy.FriendlyList) do
                friendlyCount = friendlyCount + 1
        end
        if Spy.MainWindow and Spy.MainWindow.CountFrame and Spy.MainWindow.CountFrame.FriendText then
                Spy.MainWindow.CountFrame.FriendText:SetText("|cFF00FF00" .. friendlyCount .. "|r")
        end
end
...
function Spy:ManageExpirations()
        ...
        if Spy.ManageFriendlyListExpirations then
                Spy:ManageFriendlyListExpirations()
        end
end
...
function Spy:ManageFriendlyListExpirations()
        local expired = false
        local currentTime = time()
        for player, timestamp in pairs(Spy.FriendlyList) do
                if (currentTime - timestamp) > Spy.ActiveTimeout then
                        Spy.FriendlyList[player] = nil
                        expired = true
                end
        end
        if expired then
                Spy:UpdateFriendlyCount()
        end
end
...
function Spy:AddFriendlyDetected(player, timestamp)
        if not player or player == Spy.CharacterName or SpyPerCharDB.IgnoreData[player] then
                return
        end
        local previousTimestamp = Spy.FriendlyList[player]
        local detectedTime = timestamp or time()
        Spy.FriendlyList[player] = detectedTime
        if not previousTimestamp then
                Spy:UpdateFriendlyCount()
        end
end
